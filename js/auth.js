...
function updateAuthUI(user) {
    if (user) {
        ...
        // Ensure alignment
        if (userProfile) userProfile.style.alignItems = 'center';
        ...
        if (sidebarAvatar) {
            const sName = user.fullName || 'U';
            sidebarAvatar.textContent = sName.charAt(0).toUpperCase();
        }
...
function populateProfileModal() {
    ...
    try {
        ...
        // Mock Data for specific fields if not in DB user object
        if (profileJoinDate) profileJoinDate.textContent = "Jan 2024"; // Mock date

        // Get Budget Data if available in local storage or calculate
        // (For now using mock or basic read)
        if (profileBudget) {
            // Try to read current budget from UI or localStorage if stored
            const currentBudget = localStorage.getItem('monthlyBudget') || '50000';
            profileBudget.textContent = `₹${currentBudget}`;
        }

        if (profileTotalSpent) {
            // This would ideally come from the backend or calculated from expenses
            // using a simple read from the display if possible, or placeholder
            const spentDisplay = document.getElementById('budgetSpentDisplay');
            profileTotalSpent.textContent = spentDisplay ? spentDisplay.textContent : '₹0';
        }

    } catch (e) {
        console.error("Error populating profile", e);
    }
}
